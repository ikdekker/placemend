  ...
  return (
    <div className="md:hidden fixed bottom-0 inset-x-0 z-30 bg-white/95 backdrop-blur-md border-t border-slate-200 px-3 py-2 flex items-center justify-around shadow-lg select-none pb-[max(env(safe-area-inset-bottom),8px)]">
      ...
      <button
        ...
        className="flex flex-col items-center gap-0.5 text-slate-600 hover:text-blue-600 transition-colors cursor-pointer py-1"
      >
      ...
      >
        ...
        <span className="text-[10px] font-bold">{appMode === 'edit' ? 'Editing' : 'Viewing'}</span>
      </button>

      {/* Add Furniture (Center Action) */}
      <button
        onClick={() => setFurnitureLibraryOpen(true)}
        className="flex flex-col items-center gap-0.5 -mt-3 cursor-pointer"
      >
        <div className="w-12 h-12 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center shadow-lg shadow-blue-500/30 transition-transform active:scale-95">
